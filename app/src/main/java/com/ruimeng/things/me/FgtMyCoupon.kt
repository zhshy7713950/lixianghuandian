...
import kotlinx.android.synthetic.main.fgt_my_coupon.*
...
class FgtMyCoupon : BaseBackFragment() {
    private val vm: TicketViewModel by viewModels()

    companion object {
        fun newInstance(): FgtMyCoupon {
            return FgtMyCoupon()
        }
    }

    override fun getLayoutRes(): Int = R.layout.fgt_my_coupon

    override fun onLazyInitView(savedInstanceState: Bundle?) {
        super.onLazyInitView(savedInstanceState)
        initTopbar(topbar, "我的优惠券")
    ...
    private fun initEvent() {
        vm.userPaymentInfo.observe(this@FgtMyCoupon, Observer { info ->
            if (info.deposit_status != "1" && info.rent_status != "1") {
                ToastHelper.shortToast(context, "请先完成押金支付")
            } else if (info.active_status == "2" && info.deposit_status == "1" && info.rent_status == "0") {
                FgtHome.tryToScan(AtyScanQrcode.TYPE_PAY_RENT)
            } else if (info.active_status == "1" && info.deposit_status == "1" && info.rent_status == "1") {
            ...
            } else if (info.active_status == "3" && info.deposit_status == "1" && info.rent_status == "1") {
                VoicePlayerManager.getInstance().playVoice(requireContext(), "tip-2")
                ToastHelper.shortToast(context, "请您先完成解冻操作")
            } else if (info.active_status == "2" && info.deposit_status == "1" && info.rent_status == "1") {
                FgtMain.instance?.start(
                    FgtPayRentMoney.newInstance(
                        FgtHome.CURRENT_DEVICEID,
