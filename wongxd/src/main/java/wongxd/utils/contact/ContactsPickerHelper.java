...
public class ContactsPickerHelper {
    ...
    private static final String TAG = "ContactsPickerHelper";
    /**
     * 联系人头像缓存
     */
    ...
    public static List<ContactsInfo> getContactsList(Context context) {
        ...
        if (cursor != null) {
            ...
            if (cursor.getCount() > 0) {
                Log.e(TAG, "联系人总数量:" + cursor.getCount()); //就是联系人的总数
                int count = 0;
                    ...
                    do {
                        ...
                        if (phones.isEmpty()) {
                            ...
                            for (String phone : phones) {
                                ...
                                io.name = name;
                                io.phone = phone;
                                io.letter = String.valueOf(PinyinUtils.getPinyinFirstLetter(name));
...
//                    count++;
        ...
        if (bytes == null || bytes.length <= 0) {
            ...
            if (dataCursor != null) {
                if (dataCursor.getCount() > 0) {
                    ...
                    Log.w(TAG,"数据大小:" + Formatter.formatFileSize(context, bytes.length));
                }
                dataCursor.close();
            }
        } else {
            Log.i(TAG,"缓存数据大小:" + Formatter.formatFileSize(context, bytes.length));
        }
        return bytes;
    }
    ...
    private static void log(final Cursor cursor) {
        if (cursor != null) {
            if (cursor.getCount() > 0) {
                Log.e(TAG, "----------------------start--------------------");
                Log.e(TAG, "数量:" + cursor.getCount() + " 列数:" + cursor.getColumnCount());
                if (cursor.moveToFirst()) {
                    do {
                        for (int i = 0; i < cursor.getColumnCount(); i++) {
                            ...
                            Log.e(TAG, "第" + i + "列->名称:" + columnName + " 索引:" + columnIndex + " 类型:" + ty + " 值:" + data);
                        }
                    } while (cursor.moveToNext());
                }
                Log.e(TAG, "------------------------end---------------------");
